...
def init_centroids(num_clusters, image):
    ...
    indices=[]
    for i in range(2):
        r=np.random.choice(image.shape[0])
        c=np.random.choice(image.shape[1])
        centroids_init.append(np.array([image[r,c,0],image[r,c,1],image[r,c,2]]))
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    #get assignments
    h,w,c = image.shape
    print('cccc',centroids)
    for i in range(max_iter):
        print("PROGRESS: ",i/max_iter)
        #centroid(index) to points assigned to centroid
        c2p=defaultdict(list)

        for r in range(h):
            for g in range(w):
                    p=[image[r,g,0],image[r,g,1],image[r,g,2]]
                    distances=defaultdict(int)
                    for index in range(2):
                        distances[index]=np.linalg.norm(p - centroids[index])
                    c2p[min(distances,key=distances.get)].append(p)

        centroids=[]
        for ci in range(2):
            centroids.append(np.mean(c2p[ci],axis=0))
    ...
    for r in range(h):
        for g in range(w):
                ...
                for index,c in enumerate(centroids):
                    distances[index]=np.linalg.norm(p - c)
                closestindex=min(distances,key=distances.get)
                image[r, g ,0] = centroids[closestindex][0]
                image[r, g, 1] = centroids[closestindex][1]
                image[r, g, 2] = centroids[closestindex][2]
